...
float draw_list::frametime = 0.0f;
IDirect3DDevice9 *draw_list::device = nullptr;

void draw_list::create_font ( sesui::font &font, bool force ) noexcept {
	if ( force && font.data ) {
...
void draw_list::polygon ( const std::vector< sesui::vec2 > &verticies, const sesui::color &color, bool filled ) noexcept {
...
void draw_list::multicolor_polygon ( const std::vector< sesui::vec2 > &verticies, const std::vector< sesui::color > &colors, bool filled ) noexcept {
	vertex_t *vtx = new vertex_t [ filled ? verticies.size ( ) : ( verticies.size ( ) + 1 ) ];
...
void draw_list::text ( const sesui::vec2 &pos, const sesui::font &font, const sesui::ses_string &text, const sesui::color &color ) noexcept {
	if ( !font.data )
		return;
...
void draw_list::get_text_size ( const sesui::font &font, const sesui::ses_string &text, sesui::vec2 &dim_out ) noexcept {
	RECT rect = { 0, 0, 0, 0 };
...
float draw_list::get_frametime ( ) noexcept {
	return frametime;
}

void draw_list::begin_clip ( const sesui::rect &region ) noexcept {
...
void draw_list::end_clip ( ) noexcept {
